...
void drawBricks() {
    for(int i = 0; i < 24; i++) {
        if(visibleBricks[i] == 1) {
            ...
            cout<<"****"; // gạch dài 4 ký tự
        }
    }
}

void ballHitSlider() {
    if(ballPos[1] >= sliderPos[1] && ballPos[1] <= sliderPos[1] + 9) { // slider dài 10
...
void ballHitBrick() {
    for(int i = 0; i < 24; i++) {
        if(visibleBricks[i] == 1) {
            // gạch dài 4 ký tự, nên kiểm tra đến +3
...
void play() {
    while(1) {
        ...
        if(kbhit()) {
            char ch = getch();
            if(ch == 'd' || ch == 'D' || ch == 77) {
                if(sliderPos[1] < 42) { // điều chỉnh biên phù hợp
